...
> なお LiveKit データチャネルを再利用する案（参考：§5 末尾「案A」）は「LiveKit 単体」に最も近いが、結局 ①② のために外部 DB と履歴 API を足し、③ のために送信をサーバー経由にする必要があり、④ の AV 再結合は解消されない。だから本命候補にはしていない。
...
別系統として、リアルタイム配信に新インフラを足さず **LiveKit のデータチャネルを再利用**する案。送信は Next.js Route Handler（HTTP）で受け、認可・RDS 保存のうえ **`RoomServiceClient.sendData()`** で配信、受信はクライアントの **`RoomEvent.DataReceived`**。**認可（`canSendTo`）・保存・履歴 API・DB スキーマは案E／案D と共有**でき、違いは「送信経路＝Route Handler → `sendData`」だけ。

![案A 構成](./diagrams/05-plan-a-livekit-data.svg)

| 観点 | 評価 |
|---|---|
| **メリット** | ・**追加インフラ無し**（既存 LiveKit を再利用。Socket.IO / Redis 不要）<br>・サーバー権威化で宛先をサーバーが算出（`destinationIdentitiesFor`）＝**クライアントの宛先詐称を解消**<br>・`@app/chat-core` と DB / 履歴 API を案E と共有でき、後から案E へ乗り換えても核は不変 |
...
| **実装容易性** | ○（`sendData` 配信と `DataReceived` 受信。送信経路以外は案E と同一） |

> 位置づけ：**「インフラを最小化したい」が最優先で、AV からの独立を妥協できる**場合の選択肢。逆に独立を維持したいなら案E／案D。実装サンプルは案E（[`chat-plan-e-sample.md`](./chat-plan-e-sample.md)）と核を共有し、配信部のみ差し替え。詳細は [`../CHAT_POC_FINDINGS.md` §4](../CHAT_POC_FINDINGS.md)。
