...
| 目的 | リアルタイム配信層の候補（**案A / Supabase / 案D / 案E**）を、図・メリット・デメリット・運用コスト・実装容易性で比較し、採用判断のインプットにする。 |

> 関連ドキュメント: PoC 所見は [`../CHAT_POC_FINDINGS.md`](../CHAT_POC_FINDINGS.md)。本書はその §4（構成案）を図付きで深掘りしたもの。

---

## 1. 背景：なぜ LiveKit 単体ではチャットを賄えないのか

リアルタイム配信（WebSocket の push）が要ることは自明で、しかも **LiveKit は既にそれを持っている**（Text streams / data packets を WebSocket で配信）。PoC でも全体チャットの送受信は LiveKit のデータチャネルだけで動いた。では、なぜわざわざ別のリアルタイム層（Supabase / 案D / 案E）を立てるのか。

理由は、LiveKit のデータチャネルが **「揮発的な配信」しかできず、実査チャットの中核要件を満たせない**から。PoC 所見（[`../CHAT_POC_FINDINGS.md`](../CHAT_POC_FINDINGS.md)）で確認した欠落は次の 4 つ。

![なぜ LiveKit 単体ではダメか](./diagrams/00-why-not-livekit-alone.svg)

1. **永続化がない**：LiveKit はメッセージを保存しない（公式明記）。**履歴・CSV エクスポートが成立しない**。
2. **履歴・後入室・リロードに対応できない**：ストリームは「送信時に接続済みの参加者」だけに届く（mid-stream 参加は不可）。後から入室した人・**リロードした人は過去メッセージを取得できない**——チャットとして致命的。
3. **ロール別チャネルの送信権限をサーバー強制できない**：データ送信権限は `canPublishData` の all-or-nothing で、「public には送れるが private には送れない」をトークンで表現できない。宛先（`destinationIdentities`）もクライアント任せで詐称可能。**実査の 3 階層チャネル＋ロール権限を安全に守れない**。
4. **チャットが AV ベンダーに再結合する**：現行が OpenTok signal を使わず ActionCable にしたのは、チャットを AV から独立させるため。LiveKit データチャネルに乗せると **LiveKit を差し替えるたびにチャットが巻き込まれ**、さらにチャットの生存期間が「LiveKit ルームに接続中」に縛られる（入室前・セッション外のチャットや履歴閲覧が自然に作れない）。

→ つまり必要なのは「単なるリアルタイム配信」ではなく、**永続化（記録・履歴・CSV）＋ サーバー権威の認可 ＋ AV 非依存**を備えたリアルタイム層。これを満たす候補が本書で比較する Supabase / 案D / 案E。

> なお LiveKit データチャネルを再利用する案（次の **§2 案A**）は「LiveKit 単体」に最も近い。①②③ は外部 DB・履歴 API・サーバー経由送信で補えるが、**④ の AV 再結合だけは解消されない**。AV 非依存まで求める本命候補は §3 以降の案B／案D／案E。

> **Zoom SDK でも同一**（対になる Zoom PoC `machamp0714/zoom-sdk-poc#3` の実機検証で確認）：上記 ①〜④ は Zoom Video SDK の in-session チャットでもそのまま成立し、ベンダーを置き換えても §1 の結論（**単なる realtime 配信ではなく、永続化＋サーバー権威の認可＋AV 非依存が要る**）は一字一句通る。よって Zoom SDK でも**同様に独立したリアルタイム層が必要**。
> さらに Zoom は ③④ で制約が強く、**LiveKit の `RoomServiceClient.sendData()` に相当する「サーバーからチャットを送る／webhook で受ける」経路が無い**ため、**案A（AV データチャネル再利用）相当すら作れない**。→ **Zoom SDK は案A を採れず、案B 以降（案B／案D／案E）を採用することになる**。2 つの独立 PoC（LiveKit / Zoom）が「§1 の背景は共通・結論も独立 realtime 層に収束」した、という整理になる。
...
## 2. 案A：LiveKit データチャネル再利用（参考・最小構成）

§1 の欠落のうち ①〜③ は「外部 DB ＋ サーバー権威の認可」で補えるが、**④（AV 再結合）だけは残る**——それが案A。リアルタイム配信に新インフラを足さず **LiveKit のデータチャネルを再利用**する。送信は Next.js Route Handler（HTTP）で受け、認可・RDS 保存のうえ **`RoomServiceClient.sendData()`** で配信、受信はクライアントの **`RoomEvent.DataReceived`**。**認可（`canSendTo`）・保存・履歴 API・DB スキーマは案D／案E と共有**でき、違いは「送信経路＝Route Handler → `sendData`」だけ。

![案A 構成](./diagrams/05-plan-a-livekit-data.svg)

| 観点 | 評価 |
|---|---|
| **メリット** | ・**追加インフラ無し**（既存 LiveKit を再利用。Socket.IO / Redis 不要）<br>・サーバー権威化で宛先をサーバーが算出（`destinationIdentitiesFor`）＝**クライアントの宛先詐称を解消**<br>・`@app/chat-core` と DB / 履歴 API を案D／案E と共有でき、後から乗り換えても核は不変 |
| **デメリット** | ・**チャットが AV(LiveKit) に再結合**（LiveKit 差し替え時に巻き込まれる＝そもそも分離したかった理由を失う）<br>・**受信が LiveKit ルーム接続中のみ**（入室前・セッション外のチャットは履歴 API 頼み）<br>・配信ごとに `listParticipants` で参加者ロールを取得して宛先算出が必要<br>・送受信が非対称（送信 HTTP・受信 DataReceived） |
| **運用コスト** | 低（LiveKit は AV で既に使用。チャット用の追加常駐・マネージドなし） |
| **実装容易性** | ○（`sendData` 配信と `DataReceived` 受信。送信経路以外は案D／案E と同一） |

> 位置づけ：**「インフラを最小化したい」が最優先で、AV からの独立を妥協できる**場合の選択肢。逆に AV からの独立を維持したいなら次の案B／案D／案E。実装サンプルは案E（[`chat-plan-e-sample.md`](./chat-plan-e-sample.md)）と核を共有し、配信部のみ差し替え。

---

## 3. 案B：Supabase 採用（マネージド BaaS）
...
## 4. 案D：API Gateway WebSocket + Lambda + RDS（AWS サーバーレス）
...
## 5. 案E：ECS Fargate + Socket.IO + Redis + RDS（ActionCable に最も近い）
...
> 実装案（サンプルコード）: [`chat-plan-e-sample.md`](./chat-plan-e-sample.md) — Socket.IO サーバー・認可・RDS 永続化・履歴 API・クライアントフックの参照コード。
...
| **「DB＝RDS」との整合** | ✗ 要方針変更 | ◎ | ◎ |
| **ローカル検証のしやすさ**（→ §8） | ○ Supabase CLI 一発・要 Docker | △ WS エミュ不完全 | ◎ 既存 compose 流用・local ≈ prod |

> **案A** は「AV から独立しない別系統（最小構成）」のため本表には並べていない。詳細は §2、判断軸は §7 を参照。
...
### 8-0. 既存のローカル土台（`compose.yaml`）を流用する
...
### 8-2. 案D：API Gateway WebSocket + Lambda（ローカル）
...
### 8-3. 案E：ECS Fargate + Socket.IO（ローカル）
